...
3. 当 APP 登录成功但无法换取网页 Cookie 时，不伪造 Cookie；改为保存真实 APP session，并提示部分依赖网页 Cookie 的命令可能不可用。
...
- [常见问题](#常见问题)
  ...
  * [2. 登录提示设备ID不存在或需要二次设备校验](#2-登录提示设备id不存在或需要二次设备校验)
- [交流反馈](#交流反馈)
- [鸣谢](#鸣谢)
...
## 2 登录提示设备ID不存在或需要二次设备校验
天翼账号可能开启了设备锁或触发二次校验，账号密码登录会出现设备校验、短信校验或直接失败。建议按以下顺序处理：

1. 优先使用本地维护版的 `qrlogin` 命令扫码登录：
   ```shell
   cloudpan189-go qrlogin
   ```
2. 如果仍需账号密码登录，可先在网页端登录天翼账号，检查账号安全相关的设备锁/二次校验设置。
3. 登录失败时可开启 `CLOUD189_VERBOSE=1` 获取调试日志。
